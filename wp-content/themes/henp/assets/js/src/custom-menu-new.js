...
(function () {
    ...
    function toggleNavigation() {
        return bodyClasses.contains('nav-active')
            ? closeNavigation()
            : openNavigation();
    }
    ...
    function openNavigation() {
        return (
            navClasses.add('mobile-menu--active'), bodyClasses.add('nav-active')
        );
    }
    // Close menu & remove classes
    function closeNavigation() {
        return (
            navClasses.remove('mobile-menu--active'),
            bodyClasses.remove('nav-active'),
            bodyClasses.remove('search-active')
        );
    }
    ...
    function createFocusMenu(e) {
        ...
        const menu =
            isSubmenu !== null
                ? isSubmenu
                : document.querySelector(parentMenuClass);
    ...
    function trapFocus(menu, focusableEls, activeStatus) {
        ...
        return activeStatus !== false
            ? menu.addEventListener('keydown', createFocusMenu)
            : menu.removeEventListener('keydown', createFocusMenu);
    }
    ...
    function switchActiveMenuList(e) { 
        ...
        if (typeof targetSubmenu !== 'undefined') {
            ...
            const focusableEls =
                associatedMenu.querySelectorAll(`li > a, li > button`);
            ...
            const currentActiveMenu = parentMenu.querySelector(
                '[aria-hidden="false"]'
            );
            const focusableEls =
                parentMenu.querySelectorAll(`li > a, li > button`);
    ...
    function setInitialFocus() {
        ...
        const primaryNavigation =
            document.getElementById(`mobile-menu-primary`);
        const focusableEls = primaryNavigation.querySelectorAll(
